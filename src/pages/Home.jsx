...
export const Home = () => {
    const { events, teams, loadingTeams, loadingEvents } = useOutletContext();
    ...
    // Efecto para filtrar equipos
    useEffect(() => {
        if (teams) {
            const filtered = selectedSport === "all" 
                ? teams 
                : teams.filter((team) => team.sport_id === selectedSport?.id);
    ...
    return (
        <>  
            {loadingTeams || loadingEvents ? <BlueLoader /> : (
